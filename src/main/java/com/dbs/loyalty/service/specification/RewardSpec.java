...
public class RewardSpec implements Specification<Reward>{
	...
	@Override
	public Predicate toPredicate(Root<Reward> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
		...
		if(params.containsKey(DomainConstant.CUSTOMER)) {
			predicates.add(cb.equal(root.get(DomainConstant.CUSTOMER).get(DomainConstant.ID), params.get(DomainConstant.CUSTOMER)));
		}
