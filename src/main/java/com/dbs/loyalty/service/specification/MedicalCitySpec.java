...
public class MedicalCitySpec implements Specification<MedicalCity>{
...
private static final long serialVersionUID = 1L;
	...
	@Override
	public Predicate toPredicate(Root<MedicalCity> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
		...
		if(params.containsKey(Constant.KY_PARAM) && !Constant.EMPTY.equals(params.get(Constant.KY_PARAM))) {
			...
			predicates.add(cb.or(
					...
					cb.like(cb.lower(root.get(DomainConstant.MEDICAL_PROVIDER).get(DomainConstant.NAME)), keyword)
			));
		}
