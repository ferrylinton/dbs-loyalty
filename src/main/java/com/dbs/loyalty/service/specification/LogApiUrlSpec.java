...
public class LogApiUrlSpec implements Specification<LogApiUrl> {
	...
	@Override
	public Predicate toPredicate(Root<LogApiUrl> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
		...
		if(params.containsKey(Constant.KY_PARAM) && !Constant.EMPTY.equals(params.get(Constant.KY_PARAM))) {
			...
			predicates.add(cb.like(root.get(DomainConstant.URL), keyword));
		}
