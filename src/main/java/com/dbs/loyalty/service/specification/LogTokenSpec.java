...
public class LogTokenSpec implements Specification<LogToken>{
	...
	@Override
	public Predicate toPredicate(Root<LogToken> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
		...
		if(params.containsKey(Constant.KY_PARAM) && !Constant.EMPTY.equals(params.get(Constant.KY_PARAM))) {
			...
			predicates.add(cb.like(cb.lower(root.get(DomainConstant.EMAIL)), keyword));
		}
