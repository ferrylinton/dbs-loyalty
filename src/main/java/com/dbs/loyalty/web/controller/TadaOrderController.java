...
@Controller
@RequestMapping("/tadaorder")
public class TadaOrderController {

	private static final String REDIRECT 	= "redirect:/tadaorder";
	
	private static final String VIEW 		= "tadaorder/tadaorder-view";
	
	private static final String DETAIL 		= "tadaorder/tadaorder-detail";
	...
	@GetMapping
	public String viewTadaOrders(
			@ModelAttribute(Constant.TOAST) String toast, 
			@RequestParam Map<String, String> params, 
			Sort sort, Model model) {
		
		Order order = PageUtil.getOrderDesc(sort, SORT_BY);
		Page<TadaOrder> page = tadaOrderService.findAll(params, PageUtil.getPageable(params, order));
	...
	public String viewTadaOrderDetail(ModelMap model, @PathVariable String id){
		getById(model, id);		
		return DETAIL;
	}

	private void getById(ModelMap model, String id){
		Optional<TadaOrder> current = tadaOrderService.findById(id);
		
		if (current.isPresent()) {
			model.addAttribute(DomainConstant.TADA_ORDER, current.get());
		} else {
