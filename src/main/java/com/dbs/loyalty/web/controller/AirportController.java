...
import com.dbs.loyalty.service.AirportService;
...
public class AirportController {
	...
	private static final String REDIRECT 	= "redirect:/airport";
	
	private static final String VIEW 		= "airport/airport-view";
	
	private static final String DETAIL 		= "airport/airport-detail";

	private final AirportService airportService;

	@PreAuthorize("hasAnyRole('AIRPORT_MK', 'AIRPORT_CK')")
	@GetMapping
	public String viewAirports(
		...
		Page<Airport> page = airportService.findAll(params, PageUtil.getPageable(params, order));
	...
	public String viewAirportDetail(ModelMap model, @PathVariable String id){
		getById(model, id);		
		return DETAIL;
	}

	private void getById(ModelMap model, String id){
		Optional<Airport> current = airportService.findById(id);
		
		if (current.isPresent()) {
			model.addAttribute(DomainConstant.AIRPORT, current.get());
		} else {
