...
@Controller
@RequestMapping("/medicalprovider")
public class MedicalProviderController {

	private static final String REDIRECT 	= "redirect:/medicalprovider";
	
	private static final String VIEW 		= "medicalprovider/medicalprovider-view";
	
	private static final String DETAIL 		= "medicalprovider/medicalprovider-detail";

	private final MedicalProviderService medicalProviderService;

	@PreAuthorize("hasAnyRole('MEDICAL_PROVIDER_MK', 'MEDICAL_PROVIDER_CK')")
	@GetMapping
	public String viewMedicalProviders(
			@ModelAttribute(Constant.TOAST) String toast, 
			@RequestParam Map<String, String> params, 
			Sort sort, Model model) {
		
		Order order = PageUtil.getOrder(sort, DomainConstant.NAME);
		Page<MedicalProvider> page = medicalProviderService.findAll(params, PageUtil.getPageable(params, order));
	...
	public String viewMedicalProviderDetail(ModelMap model, @PathVariable String id){
		getById(model, id);		
		return DETAIL;
	}

	private void getById(ModelMap model, String id){
		Optional<MedicalProvider> current = medicalProviderService.findById(id);
		
		if (current.isPresent()) {
			model.addAttribute(DomainConstant.MEDICAL_PROVIDER, current.get());
		} else {
